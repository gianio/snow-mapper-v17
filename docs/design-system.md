...
| **Powder Map** | The app. Everything else arrives over it and leaves again — there is no second screen and nothing slides. The mark sits top left and the account badge top right, in the same place on every surface. One column at the bottom right, where a thumb already is: community, search, locate, layers, plus — **all 48 px, all white**. Five buttons in three sizes and two colours was three decisions the eye had to make before it could pick one. The plus opens rather than acts: two circles swing out on an **arc** centred on it, Zeichnen and Beobachtung, angled left rather than up because straight up is where the rest of the column is. |
...
| **Layer panel** | Layers, and nothing else — stations, 3D and the legend live in Einstellungen, because they are properties of the device rather than of the layer. It ends with an **info card**: one line on the layer, its unit, and one line on the chosen sub-layer. A pictogram tells you which layer is live; it cannot tell you that Powder is a model estimate and not a measurement. Pictogram tiles, because a shape is quicker to re-find than a word, and the sub-layers of the chosen one open **inside the grid, directly under that tile's row**, tinted and joined to it with a notch pointing at the tile that opened them. Anywhere else and nothing would say which layer they belonged to. It closes with a 56 px ink circle at the bottom right, level with the column that opened it. |
...
| **Console** | One height, always open: the axis is the only reason the panel exists, and a control you have to unfold first is a control you use less. A card lifted off the bottom edge so terrain runs underneath it, sized so the button column clears it. Nothing else is on it — no head, no presets, no run-of-dates line — and everything left is set larger, because this is the only readout on the screen and it was sized as if it were competing with something. The window's size is printed on the window; JETZT reaches across the whole axis. |
...
| **Inspect panel** | A small **square** floating card, opened **near the middle of the screen** — you tapped a point to read it, and the middle is where reading happens — translucent and draggable, because you tapped a point to read it and a panel that fills the screen hides the terrain the reading is about. Tabular numbers, charts in ink, data palettes only for the values themselves. |
...
- **The opening view.** `_gotoCountry()` fills the screen rather than fitting
  inside it: Switzerland is wider than a phone, so fitting it leaves two dead
